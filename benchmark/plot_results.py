...
def show_image_examples(vis, images, idx='random', save=True, path='.'):
    if idx == 'random':
...
def benchmark(vis, images, save=False, path='.'):
...
images, vis, uvw, freq, cellsize, npix_x, npix_y, z = import_dataset_from_zarr(path)
idx = np.random.random_integers(len(images))


show_image_examples(vis, images, idx=220, save=True, path=f'{out}/image.png')
df = benchmark(vis, images, save=True, path=f'{out}/metrics.csv')
